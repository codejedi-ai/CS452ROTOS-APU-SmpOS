...
#   ./dev.sh run            # build + boot the kernel INTERACTIVELY (QEMU stdio terminal)
...
case "$cmd" in
  ...
  run)
    # Foreground QEMU on stdio in THIS terminal. Use `run --rm` (not `up`) so
    # docker allocates a real TTY tied to the host shell — `up` only attaches
    # via the log stream, so keystrokes can't reach QEMU's stdin.
    ensure_image
    "${DC[@]}" run --rm -it run
    ;;
